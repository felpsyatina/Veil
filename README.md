...
2. **Зайдите в веб-панель** по адресу/логину/паролю из шага 1 и создайте
   API-токен: **Settings → Security → API Token → Generate → Copy**.
   Скопируйте сразу — в некоторых версиях панель показывает токен только
   один раз.

   Это обязательно для 3x-ui **3.7.0+**: с этой версии панель по умолчанию
   отбивает обычный логин по паролю извне (HTTP 403) как небезопасный для
   API-клиентов — работает только из браузера. Токен — штатный официальный
   способ обхода этого ограничения для автоматизации, не хак.
3. В боте: **⚙️ Админ-панель → 🌍 Серверы → ➕ Добавить ноду** — мастер
   попросит имя, код страны, адрес, порт VLESS, данные панели из шага 1 и,
   последним шагом, API-токен из шага 2 (если у вас старая версия панели без
   токенов — отправьте `/skip`, тогда используется логин по паролю). Бот сам
   подключится и создаст Reality-инбаунд.
4. Готово — нода сразу автоматически выдаётся всем, у кого есть активная
