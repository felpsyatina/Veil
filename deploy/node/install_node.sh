...
ADMIN_PASS="${ADMIN_PASS:-$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 20 || true)}"
XRAY_PORT="${XRAY_PORT:-}"
...
for attempt in 1 2 3 4 5; do
  ...
  echo "Попытка ${attempt}/5 не удалась (панель, возможно, ещё стартует) — жду и пробую снова..."
  sleep 3
done
if [[ "$SETTING_OK" -ne 1 ]]; then
  echo
...
Готово!
  ...
  Панель:             http://${PUBLIC_IP}:${PANEL_PORT}
  Логин:               ${ADMIN_USER}
  Пароль:              ${ADMIN_PASS}
  Порт VLESS/Reality:  ${XRAY_PORT}

Дальше:
1. Зайдите в панель по адресу выше (именно http, TLS не настроен) и
   создайте API-токен: Settings → Security → API Token → Generate → Copy
   (скопируйте сразу — в некоторых версиях показывается один раз). Он
   понадобится в боте на последнем шаге мастера добавления ноды.
2. В Telegram-боте: ⚙️ Админ-панель → 🌍 Серверы → ➕ Добавить ноду.
   Укажите адрес этого сервера (${PUBLIC_IP}), порт ${XRAY_PORT}, данные
   панели и токен из шага 1 — бот создаст Reality-инбаунд и раздаст ноду
   всем, у кого уже есть активная подписка.

⚠️ Панель сейчас работает по HTTP без TLS-сертификата — это нормально
для админского доступа внутри дата-центра, но сохраните логин/пароль/
токен в надёжном месте и по возможности ограничьте доступ к порту
${PANEL_PORT} по IP в firewall облака.
========================================================================
SUMMARY
